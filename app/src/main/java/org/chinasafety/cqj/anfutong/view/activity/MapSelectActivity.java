...
import org.chinasafety.cqj.anfutong.presenter.impl.MapSelectPresenterImpl;
import org.chinasafety.cqj.anfutong.view.BaseActivity;
import org.chinasafety.cqj.anfutong.view.MyApplication;
import org.chinasafety.cqj.anfutong.view.fragment.CompanyMapDetailFragment;
import org.chinasafety.cqj.anfutong.view.fragment.MapCompanyListFragment;
...
//    private MapCompanyListFragment mCompanyListFragment;
    ...
    private List<CompanyDetailInfo> mDetailList;
...
//                    overlay.zoomToSpan();
    ...
    protected void setInfoToDetail(CompanyDetailInfo companyDetailInfo) {
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.map_override_view, mDetailFragment)
                .commit();
    ...
    private MapCompanyListFragment.ClickCallback mClickCallback = new MapCompanyListFragment.ClickCallback() {
        ...
        public void call(CompanyDetailInfo info) {
            if(info.getLat()==0||info.getLon()==0){
                SafeCheckActivity.start(MapSelectActivity.this, info.getId(), info.getIdStr());
                return;
            }
            try {
                for (Map.Entry<Marker, CompanyDetailInfo> entry : mMarkerList.entrySet()) {
                    if(info.getName().equals(entry.getValue().getName())){
                        setSelectIcon(entry.getKey());
                        break;
                    }
                }
